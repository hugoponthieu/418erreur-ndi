...
const UnderwaterWorld = memo((props) => {
  ...
  return (
    <Canvas
      dpr={[1, 2]}
      camera={{ position: new THREE.Vector3(0, 20, 50), fov: 30 }}
    >
...
function Ocean() {
  ...
  const color =
    colors[colors.length - Math.floor((toxicity / 100) * colors.length)];
  useEffect(() => {
    console.log(color);
  ...
  useEffect(() => {
    const generatePositionAroundCamera = () => {
      ...
      do {
        ...
        Math.abs(position[0] - camera.position.x) < 30 &&
...
function Plants({ scale, number }) {
  ...
  const [plantPositions, setPlantPositions] = useState(
    Array.from({ length: number }, () => {
      return {
        position: [Math.random() * 500 - 250, -20, Math.random() * 500 - 250],
        models: Math.floor(Math.random() * 7),
      };
    }),
  );
  ...
  return (
    <>
      {plantPositions
        .filter(
          (_, index) =>
            index < number - Math.floor((temperature / 100) * number),
        )
        .map((pos, index) => {
          // Randomly select a plant model
          const PlantModel = plantModels[pos.models];
